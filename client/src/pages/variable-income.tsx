...
export default function VariableIncomePage() {
  ...
  const confirmDelete = async () => {
    if (assetToDelete) {
      const assetToDeleteData = variableIncomeHoldings.find(h => h.id === assetToDelete.id);
    ...
    <div className="p-6 space-y-6">
      ...
      {summaryLoading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
          ...
          />
          <MetricCard
            title="Exposição Renda Variável"
            value={`${((variableIncomeHoldings.length > 0 ? (totalValue / (summary?.totalValue || 1)) : 0) * 100).toFixed(1)}%`}
            icon={TrendingUp}
          />
          <MetricCard
            title="Ativos"
