 ...
 */
async function extractStarknetBalance(
  page: any,
  walletName: string
): Promise<string | null> {
  ...
  try {
    ...
    console.error(
      `[STARKNET] Error extracting balance for ${walletName}:`,
      error
    );
    return null;
  }
 ...
 * Busca o MAIOR valor ($) encontrado na página
 */
async function extractAptosBalance(
  page: any,
  walletName: string
): Promise<string | null> {
 ...
 * Busca o MAIOR valor ($) encontrado na página
 */
async function extractSeiBalance(
  page: any,
  walletName: string
): Promise<string | null> {
...
export async function fetchBlockchainBalance(
  ...
  try {
    console.log(
      `[${config.blockchain.toUpperCase()}] [Attempt ${attempt}/${maxAttempts}] Fetching balance for ${
        config.name
      }`
    );
    ...
    try {
      ...
      if (attempt < maxAttempts) {
        console.log(
          `[${config.blockchain.toUpperCase()}] Retrying (${
            attempt + 1
          }/${maxAttempts})...`
        );
    ...
    console.error(
      `[${config.blockchain.toUpperCase()}] Error fetching balance for ${
        config.name
      }:`,
      error
    );
