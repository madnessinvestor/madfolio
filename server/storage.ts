import {
  ...
  type InsertMonthlyPortfolioSnapshot,
  assets,
  snapshots,
  monthlyStatements,
  wallets,
...
export interface IStorage {
  ...
  getMonthlyStatement(
    month: number,
    year: number
  ): Promise<MonthlyStatement | undefined>;
  createOrUpdateMonthlyStatement(
    statement: InsertMonthlyStatement
  ...
  getPortfolioHistoryByMonthYear(
    userId: string,
    month: number,
    year: number
  ): Promise<PortfolioHistory | undefined>;
  createPortfolioHistory(
    history: InsertPortfolioHistory
  ): Promise<PortfolioHistory>;
  createOrUpdatePortfolioHistory(
    history: InsertPortfolioHistory
  ): Promise<PortfolioHistory>;
  getPortfolioHistoryBySnapshots(
    userId?: string
  ): Promise<
    Array<{ date: string; totalValue: number; month: number; year: number }>
  >;
  getPortfolioHistoryByMonth(
    userId?: string
  ): Promise<
    Array<{ month: number; year: number; value: number; isLocked: number }>
  >;
  ...
  getMonthlyPortfolioSnapshot(
    userId: string,
    month: number,
    year: number
  ): Promise<MonthlyPortfolioSnapshot | undefined>;
  createOrUpdateMonthlyPortfolioSnapshot(
    snapshot: InsertMonthlyPortfolioSnapshot
  ): Promise<MonthlyPortfolioSnapshot>;
  lockMonthlySnapshot(
    snapshotId: string
  ): Promise<MonthlyPortfolioSnapshot | undefined>;
  unlockMonthlySnapshot(
    snapshotId: string
  ): Promise<MonthlyPortfolioSnapshot | undefined>;
...
export class DatabaseStorage implements IStorage {
  async getAssets(userId?: string): Promise<Asset[]> {
    if (userId) {
      return db
        .select()
        .from(assets)
        .where(and(eq(assets.userId, userId), eq(assets.isDeleted, 0)))
        .orderBy(assets.symbol);
    }
    return db
      .select()
      .from(assets)
      .where(eq(assets.isDeleted, 0))
      .orderBy(assets.symbol);
  }
  ...
  async getAssetsByMarket(market: string, userId?: string): Promise<Asset[]> {
    if (userId) {
      return db
        ...
        .where(
          and(
            eq(assets.market, market),
            eq(assets.userId, userId),
            eq(assets.isDeleted, 0)
          )
        )
        .orderBy(assets.symbol);
    }
    return db
      .select()
      .from(assets)
      .where(and(eq(assets.market, market), eq(assets.isDeleted, 0)))
      .orderBy(assets.symbol);
  }
  ...
  async getSnapshotsByDateRange(
    ...
    return db
      .select()
      .from(snapshots)
      .where(and(gte(snapshots.date, startDate), lte(snapshots.date, endDate)))
      .orderBy(desc(snapshots.date));
  }
  ...
  async createSnapshot(snapshot: InsertSnapshot): Promise<Snapshot> {
    ...
    try {
      ...
      await autoCommit(
        `feat: Add snapshot for ${date.toISOString().split("T")[0]}`
      );
      return newSnapshot;
    } catch (error) {
  ...
  async upsertSnapshot(snapshot: InsertSnapshot): Promise<Snapshot> {
    ...
    try {
      ...
      await this.updateMonthlyStatementFromSnapshots(month, year);
      await autoCommit(
        `feat: Upsert snapshot for ${date.toISOString().split("T")[0]}`
      );
      return result;
    } catch (error) {
  ...
  async createOrUpdateMonthlyStatement(
    statement: InsertMonthlyStatement
  ): Promise<MonthlyStatement> {
    const existing = await this.getMonthlyStatement(
      statement.month,
      statement.year
    );
  ...
  private async updateMonthlyStatementFromSnapshots(
    month: number,
    year: number
  ): Promise<void> {
    ...
    const monthSnapshots = await this.getSnapshotsByDateRange(
      startDate,
      endDate
    );
    ...
    const allAssets = await this.getAssets();
    for (const asset of allAssets) {
      const assetSnapshots = monthSnapshots.filter(
      ...
      if (assetSnapshots.length > 0) {
        ...
        const [latest] = await db
          ...
          .from(snapshots)
          .where(
            and(eq(snapshots.assetId, asset.id), lte(snapshots.date, endDate))
          )
          .orderBy(desc(snapshots.date))
          .limit(1);
        if (latest) {
  ...
  async getWallets(userId?: string): Promise<Wallet[]> {
    if (userId) {
      return db
        .select()
        .from(wallets)
        .where(and(eq(wallets.userId, userId), eq(wallets.isDeleted, 0)))
        .orderBy(wallets.name);
    }
    return db
      .select()
      .from(wallets)
      .where(eq(wallets.isDeleted, 0))
      .orderBy(wallets.name);
  }
  ...
  async deleteWallet(id: string): Promise<boolean> {
    try {
      ...
      console.log(
        `[SQLite] ✓ Wallet soft-deleted successfully:`,
        result[0].name
      );
  ...
  async getPortfolioHistoryByMonthYear(
    userId: string,
    month: number,
    year: number
  ): Promise<PortfolioHistory | undefined> {
  ...
  async createOrUpdatePortfolioHistory(
    history: InsertPortfolioHistory
  ): Promise<PortfolioHistory> {
    if (!history.userId) {
    ...
    const existing = await this.getPortfolioHistoryByMonthYear(
      history.userId,
      history.month,
      history.year
    );
  ...
  async getMonthlyPortfolioSnapshots(
    userId: string,
    year?: number
  ): Promise<MonthlyPortfolioSnapshot[]> {
    if (year) {
    ...
    return db
      ...
      .orderBy(
        desc(monthlyPortfolioSnapshots.year),
        monthlyPortfolioSnapshots.month
      );
  }

  async getMonthlyPortfolioSnapshot(
    userId: string,
    month: number,
    year: number
  ): Promise<MonthlyPortfolioSnapshot | undefined> {
  ...
  async createOrUpdateMonthlyPortfolioSnapshot(
    snapshot: InsertMonthlyPortfolioSnapshot
  ): Promise<MonthlyPortfolioSnapshot> {
    const userId = snapshot.userId || "default-user";
    const existing = await this.getMonthlyPortfolioSnapshot(
      userId,
      snapshot.month,
      snapshot.year
    );

    // Validação: garantir que date nunca seja null ou undefined
    if (!snapshot.date) {
      throw new Error(
        `[Storage] Cannot create/update monthly snapshot without date field for ${userId} ${snapshot.year}-${snapshot.month}`
      );
    }

    if (existing) {
      const [updated] = await db
        .update(monthlyPortfolioSnapshots)
        .set({
          totalValue: snapshot.totalValue,
          date: snapshot.date,
          isLocked: snapshot.isLocked ?? existing.isLocked,
          updatedAt: new Date(),
        })
    ...
    const [created] = await db
      .insert(monthlyPortfolioSnapshots)
      .values(snapshot)
      .returning();
    return created;
  }

  async lockMonthlySnapshot(
    snapshotId: string
  ): Promise<MonthlyPortfolioSnapshot | undefined> {
    const [updated] = await db
      .update(monthlyPortfolioSnapshots)
  ...
  async getPortfolioHistoryBySnapshots(
    userId?: string
  ): Promise<
    Array<{ date: string; totalValue: number; month: number; year: number }>
  > {
  ...
  async getPortfolioHistoryByMonth(
    userId?: string
  ): Promise<
    Array<{ month: number; year: number; value: number; isLocked: number }>
  > {
    ...
    const byMonth: Record<
      string,
      { month: number; year: number; value: number; isLocked: number }
    > = {};
